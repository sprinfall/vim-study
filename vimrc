...
" NOTE: Only used for C++ based on 'clang-format'.
" TODO: Maybe add 'for' statement.
Plug 'Chiel92/vim-autoformat'
noremap <F3> :Autoformat<CR>

" <leader>p or :Prettier[Async]
" Add @format to file for auto trigger on save.
" NOTE: Don't format JavaScript or TypeScript, just use ESLint for now.
...
" NOTE: Use 'clangcheck' instead of 'clang' because the later cannot read
" compile_commands.json (generated by CMake) for the correct compiler flags.
" NOTE: Don't lint with 'prettier' for JavaScript even it's possible.
let g:ale_linters = {
...
" NOTE: Don't fix with 'prettier' for JavaScript even it's possible.
let g:ale_fixers = {
